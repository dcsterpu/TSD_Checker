...
class Application(QWidget):
    ...
    def initUI(self, tab):
        ...
        tab.combo.addItem("Consolidated AEE2010")
    ...
    def ButtonSaveConfigClick(self):
        ...
        if self.tab1.myTextBox1.toPlainText() or self.tab1.myTextBox2.toPlainText() or self.tab1.myTextBox3.toPlainText():
            ...
            if self.tab1.combo.currentText() == "Previsional":
                ...
                data['value'] = 'Final'
            elif self.tab1.combo.currentText() == "Consolidated AEE2010":
                data['value'] = 'Consolidated AEE2010'
            list_elements.append(data)
    ...
    def ButtonLoadConfigClick(self):
        ...
        try:
            ...
            if self.list_element is not None:
                ...
                elif self.list_element["Check level"]["value"] == "Consolidated AEE2010":
                    self.tab1.combo.setCurrentIndex(3)
        ...
        if 'html' not in response.headers['content-type']:
            try:
                ...
                if self.DOC3Exists:
                    ...
                    FileName = "DOC4_template.xls"
                elif self.DOC5Exists:
                    FileName = "DOC5_template.xls"
        else:
            return "NotFile"
...
class Test(Application):
    ...
    def buttonClicked(self):
        ...
        if self.tab1.myTextBox2.toPlainText() != "" and self.tab1.myTextBox3.toPlainText() != "" and self.tab1.myTextBox31.toPlainText() == "":
            win32api.MessageBox(0, "The System field is not completed!")
            self.flag_system = True

        if not self.flag_opened_file and not self.flag_subfamily_odx and not self.flag_system:
        ...
        if self.DOC3Exists is True and not self.flag_opened_file and not self.flag_subfamily_odx:
            ...
            if self.tab2.myTextBox11.toPlainText() != "":
                ...
                if self.DOC3Name == "Error":
                    self.tab1.textbox.setText("Error: The application requires an internet connection.")
                    return
                if self.DOC3Name == "NotFile":
                    self.tab1.textbox.setText("Error: The file present at " + self.DOC3Link + " does not exist.")
                    return
            ...
            try:
                ...
                if extension == "xls":
                    ...
                    self.links = []
                    for index in range(0, len(self.DOC3Workbook.sheet_names())):
                        sheet = self.DOC3Workbook.sheet_by_index(index)
                        if sheet.name == "Report information" or sheet.name == "Test report":
                            pass
                        else:
                            for i in range(0, sheet.nrows):
                                for j in range(0, sheet.ncols):
                                    link = sheet.hyperlink_map.get((i,j))
                                    url = '(No URL)' if link is None else link.url_or_path
                                    if link is not None:
                                        cell = sheet.cell(i, j).value.replace('"','').replace("'","")
                                        if link.textmark is not None:
                                            if "'" in link.textmark:
                                                next_cell = link.textmark.split("'")
                                                next_sheet = next_cell[1].casefold()
                                                next_row_col  = next_cell[2].replace("!","")
                                                row = ""
                                                column = ""
                                                for char in next_row_col:
                                                    if char.isalpha():
                                                       column += char
                                                    else:
                                                        row += char
                                                link_good = "HYPERLINK(\"#\'" + next_sheet + "\'!$" + column + "$" + row + "\",\"" + cell + "\")"
                                                self.links.append((index, cell, i, j, link_good))
                                            else:
                                                next_cell = link.textmark.split("!")
                                                next_sheet = next_cell[0].casefold()
                                                next_row_col = next_cell[1]
                                                row = ""
                                                column = ""
                                                for char in next_row_col:
                                                    if char.isalpha():
                                                ...
                                                link_good = "HYPERLINK(\"#\'" + next_sheet + "\'!$" + column + "$" + row + "\",\"" + cell + "\")"
                                                self.links.append((index, cell, i, j, link_good))
                                        elif 'http' in link.url_or_path:
                                            click = link.url_or_path
                                            # link_good = "HYPERLINK(\"#\'" + next_sheet + "\'!$" + column + "$" + row + "\",\"" + cell + "\")"
                                            self.links.append((index, cell, i, j, click))
        ...
        if self.DOC4Exists is True and not self.flag_opened_file and not self.flag_subfamily_odx:
            ...
            if self.tab2.myTextBox12.toPlainText() != "":
                ...
                if self.DOC4Name == "Error":
                    self.tab1.textbox.setText("Error: The application requires an internet connection.")
                    return
                if self.DOC4Name == "NotFile":
                    self.tab1.textbox.setText("Error: The file present at " + self.DOC4Link + " does not exist.")
                    return
            ...
            try:
                ...
                if extension == "xls":
                    ...
                    self.links = []
                    for index in range(0, len(self.DOC4Workbook.sheet_names())):
                        sheet = self.DOC4Workbook.sheet_by_index(index)
                        if sheet.name == "Report information" or sheet.name == "Test report":
                            pass
                        else:
                            for i in range(0, sheet.nrows):
        ...
        if self.DOC5Exists is True and not self.flag_opened_file and not self.flag_subfamily_odx:
            ...
            if self.tab2.myTextBox13.toPlainText() != "":
                ...
                if self.DOC5Name == "Error":
                    self.tab1.textbox.setText("Error: The application requires an internet connection.")
                    return
                if self.DOC5Name == "NotFile":
                    self.tab1.textbox.setText("Error: The file present at " + self.DOC5Link + " does not exist.")
                    return
            ...
            try:
                ...
                if extension == "xls":
                    ...
                    self.links = []
                    for index in range(0, len(self.DOC5Workbook.sheet_names())):
                        sheet = self.DOC5Workbook.sheet_by_index(index)
                        if sheet.name == "Report information" or sheet.name == "Test report":
                            pass
                        else:
                            for i in range(0, sheet.nrows):
